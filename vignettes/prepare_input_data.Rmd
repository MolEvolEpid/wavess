...
effective population size and sampling scheme per generation (`pop_samp`), a
vector of the founder sequence(s) in character string format (`founder_seqs`),
and a nucleotide substitution rate matrix Q (`q`). In this section, we will
introduce functions that can help generate these inputs.
...
curve. By default, the starting population size (`n0`) is 1, the a carrying
capacity (`carry_cap`) is 2000, the maximum growth rate (`max_growth_rate`) is
0.35, the simulation is run for 3000 generations (`n_gen`), and a maximum of 20
sequences (`max_samp`) are sampled every 300 generations (`sampling_frequency`).
...
```{r, write_pop_samp}
write_csv(pop_samp, "input_data/pop_samp.csv")
```
...
that no multiple infections can occur. If multiple founders are provided, you
must provide the correct number for the initial population size. However, also
note that you may stochastically lose founder sequences after generation 0.
...
default, this matrix is the HIV "Q" matrix we provide with the package, which is
estimated from approximately neutral sites:
...
The required inputs related to latency are probabilities that these events
happen to a cell in one generation. All of these arguments have the prefix
`prob_`. (Note that to "turn off" any of these, all you have to do is set the
probability equal to 0.)

### Convert rates to probabilities

Much of the time, in the literature, values for events of interest are reported
as *rates*, i.e. the number of events that happen in a given amount of time.
However, our simulations require *probabilities* of an event occurring in one
generation. Therefore, we provide a function to convert a per-generation rate to
a per-generation probability:
...
```{r}
costs <- c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 0.99)
n_muts <- 0:100
...
```{r}
founder_ref$conserved <- founder_conserved_sites %>% enframe() %>% mutate(name = as.numeric(name) + 1) %>% complete(name = seq(min(name), max(name), 1L), fill = list(value='-')) %>% pull(value) %>% paste0(., collapse = '')

founder_ref$aln_cons <- identify_conserved_sites(flt_gp120,
    founder = "B.US.2011.DEMB11US006.KC473833",
    ref = "B.FR.83.HXB2_LAI_IIIB_BRU.K03455",
    founder_aln = hcf_gp120
  ) |>
  select(founder_pos, consensus_base) |>
  deframe() |>
  toupper() %>% enframe() %>% mutate(name = as.numeric(name) + 1) %>% complete(name = seq(min(name), max(name), 1L), fill = list(value='-')) %>% pull(value) %>% paste0(., collapse = '')

founder_ref$aln_cons_b <- identify_conserved_sites(
  flt_gp120[grepl('^B', labels(flt_gp120)),],
    founder = "B.US.2011.DEMB11US006.KC473833",
    ref = "B.FR.83.HXB2_LAI_IIIB_BRU.K03455",
    founder_aln = hcf_gp120
  ) |>
...
lapply(names(founder_ref), function(x) enframe(strsplit(founder_ref[[x]], '')[[1]]) %>% mutate(seq=x)) %>% bind_rows() %>% 
  pivot_wider(names_from = seq, values_from = value) %>% #View()
  filter(ref != aln_cons_b & conserved != '-')
```
