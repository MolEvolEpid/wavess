...
```{r}
gp120_start <- which(cumsum(as.character(hxb2_cons_founder['B.FR.83.HXB2_LAI_IIIB_BRU.K03455',]) != '-') == 6225)
gp120_end <- which(cumsum(as.character(hxb2_cons_founder['B.FR.83.HXB2_LAI_IIIB_BRU.K03455',]) != '-') == 7757)
```
...
len_hxb2_gp120 <- nchar(extract_seqs(hxb2_cons_founder, "B.FR.83.HXB2_LAI_IIIB_BRU.K03455", 
                                     start = gp120_start, end = gp120_end)$founder)
flt_gp120_end <- which(cumsum(as.character(hiv_env_flt_2021[1, ]) != "-") == len_hxb2_gp120)
