...
```{r}
founder_ref$conserved <- founder_conserved_sites %>%
  enframe() %>%
  mutate(name = as.numeric(name) + 1) %>%
  complete(name = seq(min(name), max(name), 1L), fill = list(value = "-")) %>%
  pull(value) %>%
  paste0(., collapse = "")
...
lapply(names(founder_ref), function(x) enframe(strsplit(founder_ref[[x]], "")[[1]]) %>% mutate(seq = x)) %>%
  bind_rows() %>%
