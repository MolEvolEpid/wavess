...
- `hiv_env_flt_2021`: the filtered HIV ENV alignment downloaded from
  the [LANL HIV sequence
  database](https://www.hiv.lanl.gov/content/sequence/NEWALIGN/align.html),
  in `ape::DNAbin` format.
...
For some reason, the data doesn't load exactly right. This is a way to get 
...
Note that the default starts with 1 active cell in generation 0, which means 
...
**TODO: IS THIS OKAY OR DO WE WANT TO MODIFY?**
...
`run_wavess()` takes as input a character vector of founder sequences
...
length.
**TODO: THINK ABOUT THE BEST WAY TO INTRODUCE MULTIPLE FOUNDER SEQUENCES (E.G. FROM DIFFERENT SUBTYPES)**
...
```{r, calc_nt_sub_probs}
calc_nt_sub_probs(as.matrix(hiv_env_flt_2021)[1:10,])
```
...
```{r, rate_to_prob}
# THIS IS ALL FOR K=1 RIGHT NOW, WHICH MIGHT BE WRONG!
rate_to_probability(0.001)
rate_to_probability(0.01)
rate_to_probability(0.1)
rate_to_probability(1, k=0) 
```
...
**MAKE SURE THE WAY WE'RE CONVERTING RATES TO PROBABILITIES IS ACCURATE**
**SHOULD WE RECOMMENT NOT USING RATES >1 BECAUSE THAT WOULD MEAN MULTIPLE EVENTS HAPPENING TO A GIVEN CELL IN ONE GENERATION**
**IS ONE ASSUMPTION THAT THE RATES ARE SMALL ENOUGH THAT IT'S NOT LIKELY FOR MORE THAN ONE EVENT TO HAPPEN TO A GIVEN CELL IN A SINGLE GENERATION?**
...
certain probabilities. Please note that this is not *exactly* how it works in
the underlying simulator, which uses a multinomial distribution to get the
number of latent cells to activate, die, and proliferate. 
**TODO: CHANGE THIS TO USE MULTINOMIAL?**

**IS THE WAY WE'RE COMPUTING THESE CORRECT? SHOULD WE BE ADDING THE PROBABILITIES TO GET THE TOTAL PROBABILITY?** Should we be getting 4 numbers from the multinomial rather than
doing the binomial first and then the multinomial?
...
#n_latent_events <- rbinom(1, latent, )
...
active_latent_counts <- lapply(pop_samp$active_cell_count, function(x){
  ...
  n_to_active <- rbinom(1, latent, prob = to_active_rate)
  n_to_proliferate <- rbinom(1, latent, prob = proliferation_rate)
  n_to_die <- rbinom(1, latent, prob = death_rate)
...
Python, we expect the indexing to begin at 1, since this is more intuitive for
R users (we account for this internally).
...
```{r}
sample_epitopes(epi_probs, ref_founder_map = ref_founder_map)
```
