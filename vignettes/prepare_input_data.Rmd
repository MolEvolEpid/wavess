...
growth curve. By default, the starting population size (`n0`) is 1, 
the a carrying capacity (`carry_cap`) is 2000, the generation at which the 
population size reaches 50% of carrying capacity (`g50`) us 25,
the simulation is run for 3000 generations (`n_gen`), 
and a maximum of 20 sequences (`max_samp`) are sampled every 300 generations 
(`sampling_frequency`).
...
The model requires as input a Q matrix (`q`), which defines the rates of nucleotide
substitution for each possible transition between nucleotides. By default, the
rate matrix is the HIV Q matrix we provide with the package: 

```{r}
hiv_q_mat
```
This matrix is based on the rates from [Zanini et al. 2017](https://doi.org/10.1093/ve/vex003),
and is converted to a Q matrix by including base frequencies estimated from
the [2022 HIV ENV filtered alignment](https://www.hiv.lanl.gov/content/sequence/NEWALIGN/align.html), 
and the overall nucleotide substitution rate estimated by [Mansky 1996](https://doi.org/10.1089/aid.1996.12.307).

**I JUST USED ENV BECAUSE IT WAS EASY, BUT MAYBE WE SHOULD USE FULL-GENOME SEQUENCES FOR THIS TO BE MORE GENERAL?**
...
Here is an example (but this matrix isn't accurate because it's simly a random
set of sequences, so use the built-in q matrix for HIV instead): 
...
contain the consensus base. **Note that this example only uses 10 sequences.
We recommend using a large alignment with very diverse sequences to
identify conserved sites to ensure that the identified
