...
pop_samp <- generate_pop_samp(n_gen = 300, sampling_frequency = 100) |>
  # sample at generation 10 to see diversity and divergence earlier on
  mutate(n_sample_active = ifelse(generation == 10, 20, n_sample_active))
...
As can be seen, the diversity of founder1 and generation 0 are NaN. This is because
...
pml_out <- pml_bb(wavess_out$seqs,
  ...
  model = "GTR+R(4)+I", rearrangement = "none"
)

tr <- root(pml_out$tree, "founder1", resolve.root = TRUE)
