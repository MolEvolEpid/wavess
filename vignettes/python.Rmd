...
that defines various parameters and includes paths to the following file inputs
...
- `nt_sub_probs`: tsv that is a named matrix of nucleotide substitution probabilities.
Rows are from, columns are to. Can be generated using the [calc_nt_sub_probs()]
function.
...
function. If this isn't provided, then conserved fitness costs are not included.
...
function [identify_conserved_sites()]. If this isn't provided, then fitness
costs relative to a reference sequence are not included. **TODO: DECIDE IF THIS
...
[get_epitope_frequencies()] and [sample_epitopes()]. If this isn't provided,
then immune fitness costs are not included. **TODO: DECIDE IF THIS IS HOW WE
WANT TO DO IT**

The parameters that must be defined in the config file are:

What fitness to use (0 is don't use, 1 is use): **TODO: DECIDE IF THIS IS HOW WE WANT TO DO IT**
...
- `prob_mut`: probability of a mutation event per site per generation
- `prob_recomb`: probability of a recombination event per site per generation
...
`main.py` takes two required inputs and one optional input:
...
```
python main.py config.yaml wavess_output/ 1234
```
