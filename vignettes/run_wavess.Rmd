...
-   **B-cell immunity**, controlled by the `epitope_locations`,
    `immune_start_day`, `n_for_imm`, and `days_full_potency` arguments.
...
population (`n_for_imm` argument). Once an epitope is recognized, it undergoes
affinity maturation for 90 days (`days_full_potency`), at which time it reaches
...
If you also specify `t_epitope_locations`, the output will additionally include
...
between the two genes is base_rate*(n+1). 
