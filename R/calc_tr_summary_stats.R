...
#' @param grps Groups from which to keep only one isolate (default: NULL, all kept)
#'
#' @return list where pure_subtree_info is a data.frame of clusters on phylogeny,
...
get_clusters <- function(tr, timepoints, pureness = 1, bootstrap = NULL, grps = NULL) {
  ...
  pure_subtrees <- get_largest_subtree(subtrs = subtrs_sub, isolate_labels = timepoints_sub, bootstrap = bootstrap, pureness = pureness)
...
get_largest_subtree <- function(subtrs, isolate_labels, control_labels = NULL, bootstrap = 90, pureness = 1) {
  ...
  largest_st_info <- lapply(names(isolate_labels), function(i) {
    ...
    # 1) HAVE ONLY A SINGLE EPI LABEL, 2) HAVE BOOTSTRAP SUPPORT GREATER THAN bootstrap, 3) INCLUDE MORE THAN ONE CONTROL LABEL
    ...
    sts <- sapply(subtrs, FUN = function(st) {
      ...
      multiple_control <- ifelse(is.null(control_labels), TRUE, # always true if not controlling for another variable
        length(unique(control_labels[intersect(st$tip.label, names(control_labels))])) > 1
