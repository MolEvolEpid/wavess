...
calc_nt_sub_probs <- function(aln, tr = NULL, model = 'GTR+R(4)+I', rearrangement = 'none'){
  ...
  # it appears that fit$Q is actually the rate matrix because it's symmetric, so have to multiply by the base frequencies to get the q matrix
  rate_mat <- matrix(c(0, fit$Q[1:3], fit$Q[1], 0, fit$Q[4:5], fit$Q[2], fit$Q[4], 0, fit$Q[6], fit$Q[3], fit$Q[5:6], 0), nrow = 4, ncol = 4)
  bf_mat <- matrix(c(fit$bf[1],rep(0,4),fit$bf[2],rep(0,4),fit$bf[3],rep(0,4),fit$bf[3]), nrow = 4, ncol = 4)
