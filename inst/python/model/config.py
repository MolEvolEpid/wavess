...
class Config:
    def __init__(self, generation_time, last_sampled_gen, founder_seqs, 
        q, mut_rate, mut_rate_scalar, recomb_rate,
        ...
        self.prob_mut, self.base_prob_mut, self.mutrate_is_sparse = self.check_sparsity(self.prob_mut, self.seq_len)
        self.mut_rate_scalar = mut_rate_scalar
        if isinstance(self.mut_rate_scalar, (float, int)):
            self.prob_mut = self.prob_mut*self.mut_rate_scalar
            self.base_prob_mut = self.base_prob_mut*self.mut_rate_scalar
            self.mut_rate_scalar = None
        # determine type of recombination breakpoint method to do
        self.prob_recomb, self.base_prob_recomb, self.recrate_is_sparse = self.check_sparsity(self.prob_recomb, self.seq_len-1)
    ...
    def calc_nt_sub_probs_from_q(self):
        ...
        sub_probs = expm(self.q * np.mean(self.mut_rate))
