...
def test_get_var_positions():
    config = Config(1, 0, {"founder0": "AAA", "founder1": "GGG"},
          ...
          None, 0, 0, 0,
          0)
    nc, bp = get_var_positions(1, 'recombination', config)
    assert nc == 1
    assert list(bp.values()) == [[2, 1]]
    nc, bp = get_var_positions(2, 'recombination', config)
    assert nc == 2
    assert list(bp.values()) == [[1, 2], [1, 2]]
    ...
    nc, bp = get_var_positions(2, 'recombination', config)
    assert nc == 1
    assert list(bp.values()) == [[1, 2]]
    config.prob_recomb = np.full(2, 0.5)
    config.base_prob = 0.5
    nc, bp = get_var_positions(5, 'recombination', config)
    assert nc == 4
    assert list(bp.values()) == [[1, 2], [1], [2], [2]]
    ...
    nc, bp = get_var_positions(1, 'recombination', config)
    assert nc == 1
    assert list(bp.values()) == [[1]]
    config.prob_recomb = np.array([0, 1])
    nc, bp = get_var_positions(1, 'recombination', config)
    assert nc == 1
    assert list(bp.values()) == [[2]]
    ...
    config.recrate_is_sparse = True
    config.base_prob = 0
    nc, bp = get_var_positions(num_cells, 'recombination', config)
    assert nc == 0
    assert list(bp.values()) == []

    # All ones: should always have maximum recombination
    config.prob_recomb = np.ones(seq_len - 1)
    nc, bp = get_var_positions(num_cells, 'recombination', config)
    assert nc >= 1
    for bplist in bp.values():
        for b in bplist:
    ...
    config.base_prob = 0
    nc, bp = get_var_positions(num_cells, 'recombination', config)
    assert len(bp.values()) == 4

    nc, bp = get_var_positions(1, 'recombination', config)
    assert nc == 1
    assert list(bp.values()) == [[2]]
    nc, bp = get_var_positions(3, 'recombination', config)
    assert nc == 0
    assert list(bp.values()) == [] 
    config.prob_recomb = 0.5
    config.base_prob = 0.5
    nc, bp = get_var_positions(6, 'recombination', config)
    assert nc == 2
    assert list(bp.values()) == [[1], [1]]
...
def test_mutate_virus_in_productive_CD4():
    seed(123)
    config = Config(1, 0, {"founder0": "AAA", "founder1": "AAA", "founder2": "AAA", "founder3": "AAA"}, 
          q, 3.5e-5, 1, 100,
          0, 0, 0, 0,
          {1: 'A'}, 0.99, "AAA", 0.1,
          pd.DataFrame({'start': [0], 'end': [3], 'maxepi': [0.3]}), 0, 10, 30,
    ...
    # host = HostEnv([HIV(seq, config)
    #                for seq in ["AAA"] * 10], 10)
    config.host.mutate_virus_in_productive_CD4({0: [1]}, config)
    assert config.host.C[0].infecting_virus.nuc_sequence == "ACA"
    assert config.host.C[0].infecting_virus.conserved_sites_mutated == set([1])
    assert config.host.C[1].infecting_virus.nuc_sequence == "AAA"
    assert config.host.C[1].infecting_virus.conserved_sites_mutated == set()
    config.host.mutate_virus_in_productive_CD4({1: [1], 3: [0]}, config)
    assert config.host.C[0].infecting_virus.nuc_sequence == "ACA"
    assert config.host.C[1].infecting_virus.nuc_sequence == "ACA"
    assert config.host.C[2].infecting_virus.nuc_sequence == "AAA"
    assert config.host.C[3].infecting_virus.nuc_sequence == "GAA"
    with pytest.raises(Exception):
        config.host.mutate_virus_in_productive_CD4(1, config)
    with pytest.raises(Exception):
        config.host.mutate_virus_in_productive_CD4([300000], config)
...
def test_get_next_gen_active():
    config = Config(1, 0, {"founder0": "AAA", "founder1": "AAA", "founder2": "AAA"}, 
          q, 3.5e-5, 1, 0,
          0, 0, 0, 0,
          {1: 'A'}, 0.99, "TTT", 0.1,
          ...
          1234)
    # host = HostEnv([HIV(seq, config) for seq in ["AAA"] * 3], 3)
    config.host.C[0].infecting_virus.mutate(0, config)
    config.host.C[1].infecting_virus.mutate(0, config)
    assert config.host.get_next_gen_active(2, 40, config) == (0, 0)
    assert [config.host.C[i].infecting_virus.nuc_sequence for i in range(2)] == [
        "TAA", "AAA"]
    assert [config.host.C[i].infecting_virus.fitness for i in range(2)] == [(1-0.1)**2, (1-0.1)**3]
    config.prob_mut = np.full(3, 0.2)
    assert config.host.get_next_gen_active(10, 40, config) == (1, 0)
    assert "TAT" in [config.host.C[i].infecting_virus.nuc_sequence for i in range(10)]
    config.prob_mut = np.full(3, 0.1)
    config.prob_recomb = np.full(2, 0.1)
    config.base_prob = 0.1
    assert  config.host.get_next_gen_active(10, 40, config) == (4, 1)
    assert "AAT" in [ config.host.C[i].infecting_virus.nuc_sequence for i in range(10)]
